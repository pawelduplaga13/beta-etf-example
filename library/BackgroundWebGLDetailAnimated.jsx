// BackgroundWebGLDetailAnimated.jsx — UNUSED archived variant.
// Same family as BackgroundWebGLDetail but the price-chart line ANIMATES:
// it drifts horizontally with sine wobble + time-varying noise, and faint
// bright tick marks travel along it. Kept around in case we want to revive
// the motion later — not loaded by BETA ETF Fund Details.html.
const { useEffect: useEffectBGDA, useRef: useRefBGDA } = React;

function BackgroundWebGLDetailAnimated() {
  const canvasRef = useRefBGDA(null);
  const rafRef = useRefBGDA(0);
  ...
  useEffectBGDA(() => {
    ...
    const fsrc = `
      ...
      // Stock-chart-like curve: ascending trend with sine + noise wobble.
      // Returns the Y position (0..1) of the curve for a given X.
      float priceCurve(float x, float t) {
        // gentle upward macro trend
        float trend = 0.30 + x * 0.30;
        // medium-frequency wobble that drifts horizontally over time
        float wobble = sin((x * 6.0) - t * 0.5) * 0.06;
        // higher-frequency noise that adds organic candle-stick feel
        float micro = snoise(vec2(x * 3.5 + t * 0.3, t * 0.4)) * 0.05;
        return trend + wobble + micro;
      }
      ...
      void main(){
        ...
        // === Drifting price-line curve ===
        // distance from this pixel to the moving curve
        ...
        // Faint tick marks moving along the curve — small bright dots every ~0.06 in uv.x
        float tickSpacing = 0.07;
        float tickPhase   = uT * 0.04;
...
window.BackgroundWebGLDetailAnimated = BackgroundWebGLDetailAnimated;
